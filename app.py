...
def process_data(fee_content, payment_content, current_sheet, next_sheet, single_month_mode=False, ref_date=None, forecast_mode=False, date_range=None):
    ...
    if forecast_mode:
        ...
        failed_sms_pmt = pd.DataFrame(columns=['Cell', 'Name', 'ID NUMBER', 'Stage', 'Amount', 'Status'])
        tracking_sms_pmt = pd.DataFrame(columns=['Cell', 'Name', 'ID NUMBER', 'Stage', 'Amount', 'Status'])
        sale_not_submitted_sms_pmt = pd.DataFrame(columns=['Cell', 'Name', 'ID NUMBER', 'Stage', 'Amount', 'Status'])
        pmt_debug = None
    else:
        try:
        ...
        def build_sms_df(src):
            if src.empty:
                return pd.DataFrame(columns=['Cell', 'Name', 'ID NUMBER', 'Stage', 'Amount', 'Status'])
            out = src[sms_cols].copy()
            out.columns = ['Cell', 'Name', 'ID NUMBER', 'Stage', 'Amount', 'Status']
            return out

        failed_sms_pmt = build_sms_df(failed_pmt_unique)
        tracking_sms_pmt = build_sms_df(tracking_pmt_unique)
        sale_not_submitted_sms_pmt = build_sms_df(sale_not_submitted_unique)
...
if not failed_sms.empty:
    ...
    edited_failed = st.data_editor(
        ...
        disabled=["Cell", "Name", "ID NUMBER", "Stage", "Amount", "Status"],
        hide_index=True,
        key="failed_editor"
    )
