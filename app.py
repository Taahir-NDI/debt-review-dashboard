...
def build_report(fee_content, payment_content):
    ...
    # ================================================================
    # ✅ SMS lists — columns now ordered: id, cell, name, stage, amount, status
    # (id first so we can dedupe, then we reorder for display later)
    # ================================================================
    sms_cols = ["id_number", "cell", "client_name", "payment_stage", "amount", "status"]
    ...
    with pd.ExcelWriter(output, engine="openpyxl") as writer:
        dashboard = pd.DataFrame({
            ...
            "Value": [
                ...
                f"{metrics['failed_cycle_count' if 'failed_cycle_count' in metrics else 'failed_cycle_c']}",
            ],
        })
        dashboard.to_excel(writer, sheet_name="Dashboard", index=False)

        # ✅ REORDER: Cell, Name, ID NUMBER, Stage, Amount, Status
        def prep(df):
            if df.empty:
                return df
...
def build_single_sheet_excel(df, sheet_name):
    ...
    with pd.ExcelWriter(output, engine="openpyxl") as writer:
        if df.empty:
            ...
            # ✅ REORDER: Cell, Name, ID NUMBER, Stage, Amount, Status
            out_df = out_df[["Cell", "Name", "ID NUMBER", "Stage", "Amount", "Status"]]
